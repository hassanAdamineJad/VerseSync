...
### 2026-09-07 — Decision: Enforce client-side setup input limits before local import

- **Context:** Local import previously accepted any chosen file and any non-empty lyric paste, so oversized audio could reach object-URL creation and waveform decoding, and huge lyric pastes could create an unusable document.
- **Choice:** Reject non-audio files and files larger than 100 MB on the setup screen before creating an audio source, keep any already valid audio selection, and block Start timing when pasted lyrics exceed 200,000 characters, 5,000 normalized non-empty lines, or 1,000 characters on any normalized line without truncating the textarea. Show inline field errors with `role="alert"` and `aria-describedby`, and reveal a character/line counter only as those limits are approached. Try sample stays independent of local field validity.
